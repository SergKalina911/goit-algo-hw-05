...
def add_contact(args, contacts):
    ...
    # якщо аргументів не два, буде викликана помилка ValueError
    name, phone = args
    
    # Додаемо новий контакт до словника contacts
    # Якщо контакт з таким ім'ям вже існує, він буде перезаписаний
    contacts[name] = phone
    return "Contact added."

@input_error
# Обгортаємо функцію change_contact для обробки помилок введення користувача 
def change_contact(args, contacts):
    ...
    # якщо аргументів не два, буде викликана помилка ValueError
    name, phone = args
    # Перевіряємо, чи існує контакт з таким ім'ям, провокуючи KeyError у випадку відсутності контакту
    contacts_phone = contacts[name]
    # Якщо контакт існує(немає помилки), оновлюємо його номер телефону
    contacts[name] = phone
    
    # повертаємо повідомлення про успішне оновлення контакту
    return f"Contact {name} is updated from {contacts_phone} to {contacts[name]}."
    
@input_error
# Декоратор input_error обгортає функцію show_phone для обробки помилок введення користувача
def show_phone(args, contacts):
    ...
    contacts (dict): Словник, де зберігаються контакти.
    Повертає:
    str: Номер телефону контакту."""
    
    # Розпаковуємо аргумент в змінну name
    # якщо аргументів менше одного, буде викликана помилка IndexError
    name = args[0]
   
    # Якщо контакт існує, повертаємо його номер телефону, якщо ні - буде викликана помилка KeyError
